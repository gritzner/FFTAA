...
class SemanticSegmentation(utils.ModelFitter):
    def __init__(self, config, params):
        ...
        for dataset in getattr(config, "evaluation_datasets", []):
            assert type(dataset) == list and len(dataset) == 2
        ...
        if config.alt_loss:
            base_loss_func = SemanticSegmentation.binary_ce_loss
        else:
            base_loss_func = nn.functional.cross_entropy
            if self.dataset.ignore_class < self.dataset.num_classes:
                config.class_weights.ignored_class_weight = self.dataset.class_weights[self.dataset.ignore_class]
        if config.metric_loss.weight > 0:
            self.loss_func = lambda yp, yt, weight, ignore_index: SemanticSegmentation.metric_loss(
                base_loss_func, config.metric_loss.weight, config.metric_loss.delta,
                yp, yt, weight, ignore_index
            )
        else:
    ...
    def pre_evaluate(self, epoch):
        ...
        if self.config.entropy_based_patch_merging_during_evaluation:
            self.eval_params.entropy_based_merging = True
            self.eval_params.predictions = {
                image_id: [
                    np.empty(self.eval_params.images[image_id].base.shape[:2], dtype=np.float64),
                    np.empty(self.eval_params.images[image_id].base.shape[:2], dtype=np.int64)
                ] for image_id in np.unique(self.eval_params.dataset.x.patch_info[:,0])
            }
            for entropy, classes in self.eval_params.predictions.values():
                entropy[:] = float("inf")
                classes[:] = self.dataset.ignore_class
        else:
            self.eval_params.entropy_based_merging = False
            self.eval_params.predictions = {
                image_id: [
    ...
    def pre_train(self, epoch, batch, batch_data):
        ...
        if getattr(self.model, "requires_dct_input", 0) > 0:
            x_dct = batch_data.x.copy()
            assert x_dct.shape[-2] == x_dct.shape[-1]
            x_dct = x_dct.reshape((
                x_dct.shape[0], x_dct.shape[1],
                self.model.encoder.feature_size, self.model.encoder.downsampling_factor,
                self.model.encoder.feature_size, self.model.encoder.downsampling_factor
            ))
            x_dct = np.transpose(x_dct, (0, 1, 3, 5, 2, 4))
            batch_data.x_dct = scipy.fftpack.dctn(x_dct, type=self.model.requires_dct_input, axes=(2, 3), norm="ortho")
    ...
    def train(self, epoch, batch, batch_data, metrics):
        ...
        inputs = {
            ...
            "ce_loss_func": self.loss_func,
            "weight": self.dataset.class_weights,
            "ignore_index": self.dataset.ignore_class
        }
        if getattr(self.model, "requires_dct_input", 0) > 0:
            inputs["x_dct"] = torch.from_numpy(batch_data.x_dct).float().to(core.device).requires_grad_()
        if not self.eval_params.enabled:
            ...
            if self.eval_params.visualize_regions:
                ...
                yp, loss = self.model(**inputs, x_vis=batch_data.x_vis, lut=self.eval_params.lut, region_visualization_path=path)
            else:
                yp, loss = self.model(**inputs)
    ...
    def post_train(self, epoch, batch, batch_data, metrics):
        if not self.eval_params.enabled:
            ...
            for yp, index_map in zip(batch_data.yp, batch_data.index_map):
                ...
                if self.eval_params.entropy_based_merging:
                    entropy = yp.copy().astype(np.float64)
                    rust.prepare_per_pixel_entropy(entropy, 10**-6)
                    entropy = np.sum(entropy, axis=0)
                    i = entropy < p[0][index_map[1],index_map[2]]
                    p[0][index_map[1][i],index_map[2][i]] = entropy[i]
                    p[1][index_map[1][i],index_map[2][i]] = np.argmax(yp, axis=0)[i]
                else:
    ...
    def finalize(self):
        ...
        subset = self.history["val_entropy"][-self.config.min_val_entropy_model_from_last_n_epochs_is_relevant:]
        relevant_epochs.add(np.argmin(subset) + (len(self.history["val_entropy"]) - len(subset)))
    ...
    def binary_ce_loss(yp, yt, weight, ignore_index):
        ...
        for c in range(yp.shape[1]):
            ...
            if c != ignore_index:
                target[yt==c] = 1
            loss.append(weight[c] * nn.functional.binary_cross_entropy_with_logits(yp[:,c], target.clone().detach()))
        return functools.reduce(lambda x,y: x+y, loss)
    
    @staticmethod
    def metric_loss(base_loss, metric_weight, delta, yp, yt, weight, ignore_index):
        yp_pos = yp.gather(1, yt.detach().unsqueeze(1))

        indices = yp.detach().topk(2, dim=1).indices
        indices = torch.where(indices[:,0]==yt, indices[:,1], indices[:,0])
        yp_neg = yp.gather(1, indices.detach().unsqueeze(1))
        
        loss = nn.functional.relu(yp_neg - yp_pos + delta)[:,0]
        loss = torch.cat([loss[yt==c] * weight[c] for c in range(yp.shape[1]) if c != ignore_index]).mean()
        
        if metric_weight < 1:
            base_loss = (1 - metric_weight) * base_loss(yp, yt, weight=weight, ignore_index=ignore_index)
            return base_loss + metric_weight * loss
