...
class PFNet(nn.Module):
    ...
    def forward(self, x, yt, ce_loss_func, weight, ignore_index):
        ...
        yp = nn.functional.interpolate(z, scale_factor=4, mode="bilinear", align_corners=False)
        
        loss = [ce_loss_func(yp, yt, weight=weight, ignore_index=ignore_index)]
        ...
        for output_stride, edge_map in zip((32, 16, 8), edge_maps):
            ...
            loss.append(nn.functional.binary_cross_entropy_with_logits(edge_map.squeeze(1), target_edge_map))
                    
        return yp, functools.reduce(lambda a,b: a+b, loss)
