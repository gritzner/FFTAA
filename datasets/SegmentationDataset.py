...
class SegmentationDataset():
    def __init__(self, config, params):
        ...
        if self.ignore_class < 0:
            # many lines of code throughout Aethon assume a non-negative index for the ignored class
        ...
        if config.hide_ignored_class:
            assert self.ignore_class == self.num_classes - 1
            self.num_classes -= 1
        
        rng = np.random.RandomState(core.random_seeds[config.random_seed])
        self.split_dataset(rng, config.split_weights)
        self.erode_class_boundaries(config.erode_class_boundaries)
    ...
    def erode_class_boundaries(self, config):
        shape = {"rect": cv.MORPH_RECT, "cross": cv.MORPH_CROSS}[config.shape.lower()]
        shape = cv.getStructuringElement(shape, (3, 3))
        
        def erode(image):
            mask = np.zeros(image.gt.shape, dtype=np.uint8)
            binary_image = np.empty_like(mask)
            for c in range(self.num_classes):
                if c == self.ignore_class:
                    continue
                binary_image[:] = 0
                binary_image[image.gt == c] = 1
                eroded_image = cv.erode(binary_image, shape)
                mask[binary_image != eroded_image] = 1
            image.gt[mask == 1] = self.ignore_class
        
        for i, image in enumerate(self.base.images):
            if config.training and i < self.split[1]:
                erode(image)
            elif config.validation and self.split[1] <= i and i < self.split[2]:
                erode(image)
            elif config.test and self.split[2] <= i:
                assert i < self.split[3]
                erode(image)
        
    def compute_weights_and_normalization_params(self, config):
        ...
        for i in range(*self.split[:2]):
            ...
            for c in range(self.num_classes):
                self.class_counts[c] += np.count_nonzero(gt == c)
        ...
        cov = utils.CovarianceMatrix(self.base.images[0].base.shape[2]+1, True)
        for i in range(*self.split[:2]):
            img = self.base.images[i]
            cov.add_to_mean(img.base, img.depth)
        cov.finalize_mean()
        for i in range(*self.split[:2]):
            img = self.base.images[i]
            cov.add_to_cov(img.base, img.depth)
        cov.finalize_cov()

        mean, cov, corr = cov.mean, cov.cov, cov.corr
        if hasattr(config, "fixed_depth_std"):
            cov[-1, -1] = config.fixed_depth_std**2
        np.savez_compressed(
            f"{core.output_path}/{self.name}_normalization_params.npz",
            mean = mean,
            cov = cov,
            corr = corr
        )
        
        if config.independent_channels.lower() == "all":
            cov = np.diag(np.diag(cov))
            corr = np.diag(np.diag(corr))
        elif config.independent_channels.lower() == "last":
            cov[-1, :-1] = 0
            cov[:-1, -1] = 0
            corr[-1, :-1] = 0
            corr[:-1, -1] = 0
        else:
            assert config.independent_channels.lower() == "none"
        self.normalization_params = types.SimpleNamespace(
            mean = mean.astype(np.float32),
            cov = cov,
            corr = corr,
            whitening = utils.compute_whitening_matrix(cov, corr, config.method).astype(np.float32)
        )
    ...
    def get_image_view(self, image_id):
        patch_size = list(self.test.x.shape[2:])
        test_time_augmentation = self.test.x.test_time_augmentation
        patch_info = self.split_images(image_id, image_id+1, patch_size, .5 if test_time_augmentation else 0)
        return types.SimpleNamespace(
            x = NormalizedInputProvider(self, patch_info, self.base.input_channels, patch_size, test_time_augmentation),
            index_map = IndexMapProvider(patch_info, patch_size, test_time_augmentation)
        )
    
    def get_image_subset(self, image_id):
        if image_id < self.split[1]:
            return "training", "train", 0
        elif image_id < self.split[2]:
            return "validation", "val", 1
        return "test", "test", 2
    
    def add_oracle(self):
        self.training.oracle = OracleProvider(self)
