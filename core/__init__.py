...
def open(filename, mode):
    if filename[-3:] == ".xz":
        return lzma.open(filename, mode, preset=None if "r" in mode else 9)
    elif filename[-3:] == ".gz":
...
def init(args=None):
    ...
    config_yaml = raw_yaml.split("\n")
    aethon_array_indices = None, None
    for i, line in enumerate(config_yaml):
        if line.strip() == "#AETHON_ARRAY_BEGIN":
            assert aethon_array_indices[0] is None
            aethon_array_indices = i, aethon_array_indices[1]
        elif line.strip() == "#AETHON_ARRAY_END":
            assert aethon_array_indices[1] is None
            aethon_array_indices = aethon_array_indices[0], i
    if not (aethon_array_indices[0] is None and aethon_array_indices[1] is None):
        assert not (aethon_array_indices[0] is None)
        assert not (aethon_array_indices[1] is None)
        assert aethon_array_indices[0] < aethon_array_indices[1]
        config_yaml = config_yaml[:aethon_array_indices[0]] + config_yaml[aethon_array_indices[1]+1:]
    config_yaml = "\n".join(config_yaml)
